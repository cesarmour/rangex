...
export async function buildCompleto({ sessions, totals, sessionsPlatformCount, precos, club }) {
  ...
  for (let i = 0; i < sessions.length; i++) {
    ...
    doc.text('Análise por quadrante', colRightX, yRight)
    yRight += 5

    doc.setFontSize(9)
    const quadrantes = [
      ['Amarelo', s.quadrantes?.amarelo || '—'],
      ['Verde', s.quadrantes?.verde || '—'],
      ['Vermelho', s.quadrantes?.vermelho || '—'],
      ['Azul', s.quadrantes?.azul || '—'],
    ]
    for (const [nome, texto] of quadrantes) {
      doc.setFont('helvetica', 'bold')
      doc.text(`${nome}:`, colRightX, yRight)
      const labelW = doc.getTextWidth(`${nome}: `)
      doc.setFont('helvetica', 'normal')
      const lines = doc.splitTextToSize(texto, colRightW - labelW)
      doc.text(lines[0], colRightX + labelW, yRight)
      yRight += 4
      for (let k = 1; k < lines.length; k++) {
        doc.text(lines[k], colRightX, yRight)
        yRight += 4
      }
      yRight += 1
    }

    const yBelow = Math.max(yLeft, yRight) + 4
    y = drawSectionHeader(doc, yBelow, 'Diagnóstico técnico')
    drawParagraph(doc, y, s.diagnostico || 'Diagnóstico não preenchido.')
  }
