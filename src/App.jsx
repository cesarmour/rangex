...
import {
  ...
  loadTrainings, saveTraining, deleteTraining,
} from './lib/db.js'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        let userAcervo = await loadAcervo(user.id)
        if (userAcervo.length === 0) {
          // Idempotent: server-side flag prevents re-seeding
          await seedDefaultAcervo(DEFAULT_ACERVO)
          userAcervo = await loadAcervo(user.id)
        }
        const userTrainings = await loadTrainings(user.id)
        if (cancelled) return
        setProfile(p)
        ...
        hydrateRef.current = false // allow retry on error
        alert('Erro ao carregar seus dados: ' + e.message)
      }
    })()
    ...
    for (let i = 0; i < idsToAnalyze.length; i++) {
      ...
      try {
        ...
        setSessions((prev) => prev.map((s) =>
          s.id === id
            ? { ...s, disparos: result.disparos, pontos: result.pontos, quadrantes: result.quadrantes, diagnostico: result.diagnostico, analyzed: true }
            : s
        ))
      } catch (e) {
  ...
  const loadTrainingFromHistory = (id) => {
    ...
    if (t.trainedAt) setTrainingDateTime(toDateTimeLocal(t.trainedAt))
    if (t.club) setClub(t.club)
      ...
      {activeTab === 'treino' && (
        <main className="max-w-2xl mx-auto px-4 py-5 space-y-4">
          ...
          <div className="space-y-3">
            {sessions.map((s, i) => (
              <SessionCard key={s.id} session={s} index={i} acervo={acervo}
